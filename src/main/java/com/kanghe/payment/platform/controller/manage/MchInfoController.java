...
import com.kanghe.payment.platform.service.MchInfoService;
...
public class MchInfoController {
    ...
    public String list(@ModelAttribute MchInfo mchInfo, Integer pageIndex, Integer pageSize) {
        ...
        int count = mchInfoService.getCount(mchInfo);
        if (count <= 0) {
            return JSON.toJSONString(pageModel);
        }
        List<MchInfo> mchInfoList = mchInfoService.getList((pageIndex - 1) * pageSize, pageSize, mchInfo);
